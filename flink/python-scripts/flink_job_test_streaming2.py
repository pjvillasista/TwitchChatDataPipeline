...
def wait_for_rest_catalog():
    import requests
    from time import sleep
    max_retries = 30
    for i in range(max_retries):
        try:
            response = requests.get('http://rest:8181/v1/config')
...
# Call before catalog setup
if not wait_for_rest_catalog():
    raise Exception("REST catalog not available")
...
def setup_iceberg_catalog(t_env):
    ...
    CREATE TABLE IF NOT EXISTS iceberg_catalog.raw_db.raw_messages (
        ...
        'write.merge.max-concurrent-file-group-rewrites' = '2',
        'write.merge.max-file-groups-per-rewrite' = '2',
        'write.commit.interval' = '5min'
    )"""
